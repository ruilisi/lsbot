...
WeCom Cloud Relay:
  ...
  2. Configure callback URL in WeCom: https://lsbot.net/wecom  (INT: https://lsbot.org/wecom)
...
func init() {
	...
	relayCmd.Flags().StringVar(&relayHost, "host", "", "Base URL of the relay server (e.g. http://localhost:8080 or https://lsbot.net); sets --server and --webhook")
	relayCmd.Flags().StringVar(&relayServerURL, "server", "", "WebSocket URL (default: wss://lsbot.net/ws, or RELAY_SERVER_URL env; INT: wss://lsbot.org/ws)")
	relayCmd.Flags().StringVar(&relayWebhookURL, "webhook", "", "Webhook URL (default: https://lsbot.net/webhook, or RELAY_WEBHOOK_URL env; INT: https://lsbot.org/webhook)")
...
func runRelay(cmd *cobra.Command, args []string) {
	...
	if relayPlatform == "wecom" {
		...
		if len(missing) > 0 {
			...
			fmt.Fprintln(os.Stderr, "Configure callback URL in WeCom: https://lsbot.net/wecom  (INT: https://lsbot.org/wecom)")
			os.Exit(1)
		}
	}
